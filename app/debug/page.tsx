...
export default function DebugPage() {
  ...
  const handleSync = async () => {
    try {
      ...
      toast.success(
        `Synchronisation terminée : ${result.synced} sprint(s) mis à jour`
      );
  ...
  const sprint25 = sprints.find(
    (sprint) =>
      sprint.title.includes("Sprint 25") || sprint.title.includes("25")
  );
    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold">
          Debug - État des Sprints et User Stories
        </h1>
      ...
      <div className="bg-gray-100 p-4 rounded">
        <h2 className="text-lg font-semibold mb-2">
          Sprints marqués comme actifs (isActive: true)
        </h2>
        {activeSprints.length === 0 ? (
          ...
          activeSprints.map((sprint) => (
            <div key={sprint.id} className="mb-4 p-3 bg-white rounded border">
              ...
              <p>
                UserStoryIds dans le sprint: [
                {sprint.userStoryIds?.join(", ") || "aucun"}]
              </p>
            </div>
          ))
        )}
      ...
      {sprint25 && (
        <div className="bg-blue-100 p-4 rounded">
          ...
          <div className="p-3 bg-white rounded border">
            ...
            <p>
              UserStoryIds dans le sprint: [
              {sprint25.userStoryIds?.join(", ") || "aucun"}]
            </p>
          </div>
        </div>
      )}
          ...
          return us ? (
            ...
            >
              ...
              <span className="text-gray-600">
                SprintId: {us.sprintId || "❌ aucun"}
              </span>
            </div>
          ) : (
            <div
              key={code}
              className="mb-2 p-2 bg-white rounded border text-sm text-red-500"
            >
              {code} - ❌ Non trouvé
            </div>
          );
